...
describe('Datetime', () => {
	...
	describe('with set initialRenderDate', () =>{
		it('will render a date in the past when opened', () => {
			const date = new Date(2000, 0, 1, 2, 2, 2, 2),
				component = utils.createDatetime({ initialRenderDate: date });
				utils.openDatepicker(component);
			expect(component.find('.rdtSwitch').text()).toEqual('January 2000');
		});
		it('will render a date in the future when opened', () => {
			const date = new Date(2020, 0, 1, 2, 2, 2, 2),
				component = utils.createDatetime({ initialRenderDate: date });
				utils.openDatepicker(component);
			expect(component.find('.rdtSwitch').text()).toEqual('January 2020');
		});
	});
});
