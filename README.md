...
| **datetimeFormat**   | `boolean` or `string`  | `false` | Defines the format for the datetime displayed as value. It accepts any [Moment.js date format](http://momentjs.com/docs/#/displaying/format/) (not in localized format). If `false` the date will be displayed using the defaults for the current locale (according to `dateFormat` and `timeFormat`). |
