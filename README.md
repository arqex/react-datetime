...
| **prevButtonProps** | `object` or `function` | {} | Defines additional attributes for the previous period button at the top of the datepicker. If a function is passed, it will receive the current view mode (`string`) as an argument, and return of the function will be used as props to pass to the DOM node.
| **switchButtonProps** | `object` or `function` | {} | Defines additional attributes for the "switch" button at the top of the datepicker which displays the current selection and takes the user back to previous view mode when clicked. If a function is passed, it will receive the current view mode (`string`) as an argument, and return of the function will be used as props to pass to the DOM node.
| **nextButtonProps** | `object` or `function` | {} | Defines additional attributes for the next period button at the top of the datepicker. If a function is passed, it will receive the current view mode (`string`) as an argument, and return of the function will be used as props to pass to the DOM node.
| **tileProps** | `object` or `function` | {} | Defines additional attributes for datepicker calendar tiles. If a function is passed, it will receive the current view mode (`string`), Moment object corresponding to the tile and isDisabled flag as arguments, and return of the function will be used as props to pass to the DOM node.
